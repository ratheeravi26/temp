# Databricks Notebook: Bronze Ingestion for GroupDetails_v0
# Purpose: Ingests raw GroupDetails_v0 data from ADLS Gen2 into a partitioned Bronze Delta table.
...
dbutils.widgets.text("input_base_path", "/mnt/adls_raw/m365/groupdetails_v0", "Base Input Path for GroupDetails_v0")
dbutils.widgets.text("bronze_database_name", "m365_bronze", "Bronze Layer Database Name")
dbutils.widgets.text("bronze_table_name", "groupdetails_v0", "Bronze Layer Table Name for GroupDetails")
...
from pyspark.sql.types import StructType, StructField, StringType, BooleanType, ArrayType, TimestampType, LongType
from datetime import datetime

# COMMAND ----------
# DBTITLE 4,Helper Functions and Initial Setup
def get_logger():
    # In a real production scenario, you might integrate with a more sophisticated logging framework.
    # For this notebook, we'll use print statements prefixed with a timestamp.
    def log(message):
...
# Validate snapshot_date_str format
try:
...
# COMMAND ----------
# DBTITLE 5,Define Source Schema for GroupDetails_v0
# Based on Microsoft Graph Data Connect documentation for GroupDetails_v0
# Ref: https://github.com/microsoftgraph/dataconnect-solutions/blob/main/Datasets/data-connect-dataset-groupdetails.md
group_details_schema = StructType()
...
try:
    ...
    raw_df = spark.read.format("json") \
       .schema(group_details_schema) \
       .load(input_path)

    # Check if DataFrame is empty after loading (e.g., path exists but contains no valid JSON files)
...
try:
    ...
    (bronze_df.write
       .format("delta")
       .mode("overwrite") # Overwrite specific partitions for idempotency
       .option("replaceWhere", f"snapshot_date = '{snapshot_date_str}'") # Condition for replacing data
...
try:
    # OPTIMIZE can be run to compact small files, especially if many small files are generated.
    # For daily batch loads, this might be scheduled less frequently or based on table size/file count.
    # logger.info(f"Optimizing table {full_bronze_table_name} for partition snapshot_date = '{snapshot_date_str}'")
    # spark.sql(f"OPTIMIZE {full_bronze_table_name} WHERE snapshot_date = '{snapshot_date_str}'")
    
    # VACUUM should be run with caution and typically with a retention period longer than default (7 days)
    # if time travel for longer periods is required.
    # spark.sql(f"VACUUM {full_bronze_table_name} RETAIN 168 HOURS") # Example: 7 days retention

    logger.info(f"Bronze ingestion for {full_bronze_table_name} completed for snapshot date: {snapshot_date_str}")
except Exception as e:
    logger.warning(f"Post-write operations for {full_bronze_table_name} encountered an error: {str(e)}")

# COMMAND ----------
dbutils.notebook.exit("Successfully completed GroupDetails_v0 Bronze ingestion.")